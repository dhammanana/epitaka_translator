"""
ai_jobs/sinhala_job/si_data.py  —  Data-access layer for SinhalaTranslatorJob.

Design rule: every function is self-contained.
  • It resolves the DB path from config / params.
  • Opens a connection with a context manager (with … as conn).
  • Fetches or writes what it needs.
  • The connection is closed automatically when the with block exits.

No connection object is ever passed in or stored between calls.

Source databases
----------------
  sinhala.db       — the authentic Sinhala translation published in the
                     ePiṭaka project.  Read-only.  Contains:
                       entries(filename, page_num, page_order, entry_order,
                               type, level, palitext, sinhalatext)
                       epitaka_link(sinhala_filename, sinhala_page_num,
                                    sinhala_entry_order, sinhala_palitext,
                                    nissaya_book_id, nissaya_para_id,
                                    nissaya_title, match_confidence)
  nissaya.db       — the Nissaya source we are translating.  Read/write.
                     Contains: books, headings, sentences, nissaya,
                               pali_definition.
  epitaka_si.db    — output database.  Sentences translated to Sinhala
                     are written to the `sentences` table here.
  glossary.db      — sinhala_glossary table (created on first use).

Public API
----------
  # path helpers
  nissaya_path(params) → str
  sinhala_path(params) → str
  output_path(params)  → str

  # startup validation
  validate_nissaya_db(params, log_info, log_warn)
  validate_sinhala_db(params, log_info, log_warn)
  ensure_output_db(params, log_info)
  ensure_sinhala_glossary_table(log_info)

  # reads
  fetch_headings(params, log_info)                                → list[dict]
  fetch_paragraphs_for_heading(params, heading, overwrite,
                               log_info, log_warn)               → list[dict]
  fetch_sinhala_reference(params, book_id, para_id,
                          log_info, log_warn)                    → dict
  fetch_nissaya_map(params, book_id, para_id, log_info)          → dict[int, str]
  build_nissaya_block(sentences, nissaya_map)                    → str
  fetch_glossary_block(pali_tokens, log_info, log_warn)          → str
  fetch_previous_paragraph_translation(params, book_id, para_id,
                                        log_info)                → str
  fetch_pali_definitions_block(pali_text, params,
                               log_info, log_warn)               → str

  # writes
  save_translations(params, book_id, para_id,
                    translations, log_info, log_warn, log_error) → int
  upsert_glossary(terms, source_id,
                  log_info, log_warn, log_error)                 → int
"""

import re
import sqlite3
import logging
from contextlib import contextmanager
from typing import Callable

from database import get_glossary_conn
from config import NISSAYA_DB

logger = logging.getLogger(__name__)

_Log = Callable[[str], None]

# ── Defaults (overridable via params) ────────────────────────────────────────

_DEFAULT_SINHALA_DB = "data/sinhala.db"
_DEFAULT_OUTPUT_DB  = "data/epitaka_si.db"


# ══════════════════════════════════════════════════════════════════════════════
# Path resolvers
# ══════════════════════════════════════════════════════════════════════════════

def nissaya_path(params: dict) -> str:
    path = params.get("nissaya_db") or NISSAYA_DB
    if not path:
        raise RuntimeError(
            "nissaya_db path is not configured. "
            "Set NISSAYA_DB in config.py or pass it as a task param."
        )
    return str(path)


def sinhala_path(params: dict) -> str:
    path = params.get("sinhala_db") or _DEFAULT_SINHALA_DB
    return str(path)


def output_path(params: dict) -> str:
    path = params.get("output_db") or _DEFAULT_OUTPUT_DB
    return str(path)


# ══════════════════════════════════════════════════════════════════════════════
# Internal connection helper
# ══════════════════════════════════════════════════════════════════════════════

@contextmanager
def _connect(path: str, read_only: bool = False):
    """Open a SQLite connection, yield it, always close on exit."""
    uri = f"file:{path}?mode=ro" if read_only else str(path)
    conn = sqlite3.connect(uri if read_only else path, timeout=30,
                           uri=read_only)
    conn.row_factory = sqlite3.Row
    conn.execute("PRAGMA journal_mode=WAL")
    conn.execute("PRAGMA busy_timeout=10000")
    try:
        yield conn
    finally:
        conn.close()


# ══════════════════════════════════════════════════════════════════════════════
# Startup validation
# ══════════════════════════════════════════════════════════════════════════════

def validate_nissaya_db(params: dict, log_info: _Log, log_warn: _Log) -> None:
    path = nissaya_path(params)
    with _connect(path) as conn:
        tables = {
            r[0] for r in
            conn.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()
        }
    log_info(f"[DB] nissaya.db at {path!r}. Tables: {sorted(tables)}")
    for t in ("books", "headings", "sentences", "nissaya", "pali_definition"):
        if t not in tables:
            log_warn(f"[DB] nissaya.db MISSING expected table: '{t}'")


def validate_sinhala_db(params: dict, log_info: _Log, log_warn: _Log) -> None:
    path = sinhala_path(params)
    with _connect(path) as conn:
        tables = {
            r[0] for r in
            conn.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()
        }
    log_info(f"[DB] sinhala.db at {path!r}. Tables: {sorted(tables)}")
    for t in ("entries", "epitaka_link"):
        if t not in tables:
            log_warn(f"[DB] sinhala.db MISSING expected table: '{t}'")


def ensure_output_db(params: dict, log_info: _Log) -> None:
    """
    Create epitaka_si.db and the `sentences` table if they don't exist.
    The table mirrors the structure of nissaya.sentences but stores
    the Sinhala translation in `sinhala_translation`.
    """
    path = output_path(params)
    with _connect(path) as conn:
        conn.execute("""
            CREATE TABLE IF NOT EXISTS sentences (
                id                   INTEGER PRIMARY KEY AUTOINCREMENT,
                book_id              TEXT    NOT NULL,
                para_id              INTEGER NOT NULL,
                line_id              INTEGER NOT NULL,
                pali_sentence        TEXT,
                sinhala_translation  TEXT,
                created_at           TEXT    DEFAULT (datetime('now')),
                updated_at           TEXT    DEFAULT (datetime('now')),
                UNIQUE (book_id, para_id, line_id)
            )
        """)
        conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_si_book_para
            ON sentences(book_id, para_id)
        """)
        conn.commit()
    log_info(f"[DB] Output DB ready at {path!r}.")


def ensure_sinhala_glossary_table(log_info: _Log) -> None:
    """
    Create `sinhala_glossary` table in glossary.db if it doesn't exist.
    Shares the glossary.db file with the English glossary but lives in
    its own table.
    """
    try:
        conn = get_glossary_conn()
        conn.execute("""
            CREATE TABLE IF NOT EXISTS sinhala_glossary (
                id         INTEGER PRIMARY KEY AUTOINCREMENT,
                pali       TEXT NOT NULL,
                sinhala    TEXT NOT NULL,
                domain     TEXT DEFAULT '',
                sub_domain TEXT DEFAULT '',
                context    TEXT DEFAULT '',
                note       TEXT DEFAULT '',
                source_id  TEXT DEFAULT '',
                created_at TEXT DEFAULT (datetime('now')),
                UNIQUE (pali, sinhala)
            )
        """)
        conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_si_gloss_pali
            ON sinhala_glossary(pali)
        """)
        conn.commit()
        conn.close()
        log_info("[DB] sinhala_glossary table is ready.")
    except Exception as exc:
        log_info(f"[DB] Could not ensure sinhala_glossary table: {exc}")


# ══════════════════════════════════════════════════════════════════════════════
# Headings — fetch work items from nissaya.db
# ══════════════════════════════════════════════════════════════════════════════

def fetch_headings(params: dict, log_info: _Log) -> list[dict]:
    """
    Return headings that still have sentences pending Sinhala translation.

    Params consumed: book_id, heading_id (= sc_id alias), batch_size, overwrite.

    A heading is considered "done" when every sentence in its paragraph range
    already has a sinhala_translation row in epitaka_si.db.
    """
    book_id_filter   = params.get("book_id",    "").strip()
    heading_id_filter = params.get("sc_id",     "").strip()
    batch_size        = int(params.get("batch_size", 5))
    overwrite         = bool(params.get("overwrite", False))

    log_info(
        f"[DB] fetch_headings — book_id={book_id_filter!r}, "
        f"sc_id={heading_id_filter!r}, batch_size={batch_size}, "
        f"overwrite={overwrite}"
    )

    niss_path = nissaya_path(params)
    out_path  = output_path(params)

    try:
        with _connect(niss_path) as conn:
            if heading_id_filter:
                rows = conn.execute(
                    "SELECT * FROM headings WHERE sc_id = ?",
                    (heading_id_filter,)
                ).fetchall()
            elif book_id_filter:
                rows = conn.execute(
                    """SELECT * FROM headings
                       WHERE book_id = ?
                       ORDER BY para_id""",
                    (book_id_filter,)
                ).fetchall()
            else:
                rows = conn.execute(
                    "SELECT * FROM headings ORDER BY book_id, para_id"
                ).fetchall()
            candidates = [dict(r) for r in rows]

        log_info(f"[DB] fetch_headings: {len(candidates)} candidate heading(s).")

        if overwrite or not candidates:
            return candidates[:batch_size]

        # Filter out headings that are already fully translated in output DB.
        result = []
        with _connect(out_path) as out_conn:
            for h in candidates:
                if len(result) >= batch_size:
                    break
                start = h["para_id"]
                end   = start + int(h.get("chapter_len") or 1)

                # Count sentences in nissaya that exist for this heading range
                with _connect(niss_path) as nc:
                    total_row = nc.execute(
                        """SELECT COUNT(*) FROM sentences
                           WHERE book_id = ? AND para_id >= ? AND para_id < ?""",
                        (h["book_id"], start, end)
                    ).fetchone()
                total = total_row[0] if total_row else 0
                if total == 0:
                    continue  # heading has no sentences at all

                # Count how many are already translated in output DB
                done_row = out_conn.execute(
                    """SELECT COUNT(*) FROM sentences
                       WHERE book_id = ? AND para_id >= ? AND para_id < ?
                         AND sinhala_translation IS NOT NULL
                         AND sinhala_translation != ''""",
                    (h["book_id"], start, end)
                ).fetchone()
                done = done_row[0] if done_row else 0

                if done < total:
                    result.append(h)

        log_info(f"[DB] fetch_headings: {len(result)} heading(s) need work.")
        return result

    except Exception as exc:
        log_info(f"[DB] fetch_headings EXCEPTION — {type(exc).__name__}: {exc}")
        raise


# ══════════════════════════════════════════════════════════════════════════════
# Paragraphs & sentences
# ══════════════════════════════════════════════════════════════════════════════

def fetch_paragraphs_for_heading(
    params:    dict,
    heading:   dict,
    overwrite: bool,
    log_info:  _Log,
    log_warn:  _Log,
) -> list[dict]:
    """
    A heading covers para_id .. para_id + chapter_len - 1.

    Returns a list of paragraph dicts:
      {
        "book_id":   str,
        "para_id":   int,
        "sentences": [ {"line_id": int, "pali_sentence": str} ],
        "pending":   [ {"line_id": int, "pali_sentence": str} ]
      }

    "pending" contains only sentences without an existing Sinhala translation
    in epitaka_si.db (unless overwrite=True).
    """
    niss_path = nissaya_path(params)
    out_path  = output_path(params)

    start_para  = heading["para_id"]
    chapter_len = int(heading.get("chapter_len") or 1)
    end_para    = start_para + chapter_len
    book_id     = heading["book_id"]

    log_info(
        f"[DB] fetch_paragraphs_for_heading: {book_id} "
        f"para_id={start_para}..{end_para - 1}"
    )

    result = []
    with _connect(niss_path) as nconn:
        with _connect(out_path) as oconn:
            for pid in range(start_para, end_para):
                rows = nconn.execute(
                    """SELECT line_id, pali_sentence
                       FROM sentences
                       WHERE book_id = ? AND para_id = ?
                       ORDER BY line_id""",
                    (book_id, pid)
                ).fetchall()

                sentences = [dict(r) for r in rows]
                if not sentences:
                    continue

                if overwrite:
                    pending = sentences
                else:
                    # Build set of already-translated line_ids from output DB
                    done_rows = oconn.execute(
                        """SELECT line_id FROM sentences
                           WHERE book_id = ? AND para_id = ?
                             AND sinhala_translation IS NOT NULL
                             AND sinhala_translation != ''""",
                        (book_id, pid)
                    ).fetchall()
                    done_ids = {r["line_id"] for r in done_rows}
                    pending  = [s for s in sentences if s["line_id"] not in done_ids]

                if not pending and not overwrite:
                    continue

                result.append({
                    "book_id":   book_id,
                    "para_id":   pid,
                    "sentences": sentences,
                    "pending":   pending,
                })

    log_info(
        f"[DB] fetch_paragraphs_for_heading: {len(result)} para(s) with "
        f"{sum(len(p['pending']) for p in result)} pending sentence(s)."
    )
    return result


# ══════════════════════════════════════════════════════════════════════════════
# Sinhala reference text  (replaces SC reference)
# ══════════════════════════════════════════════════════════════════════════════

def fetch_sinhala_reference(
    params:   dict,
    book_id:  str,
    para_id:  int,
    log_info: _Log,
    log_warn: _Log,
) -> dict:
    """
    Use epitaka_link in sinhala.db to find the authentic Sinhala (and Pāli)
    entries that correspond to this nissaya book_id / para_id.
    Returns:
      {
        "title":       str,         — heading title from the link row
        "pali_block":  str,         — all matched Pāli entries joined
        "sinhala_block": str,       — all matched Sinhala entries joined
      }
    Falls back gracefully if no link is found.
    """
    si_path = sinhala_path(params)
    try:
        with _connect(si_path) as conn:
            # Find matching entries via epitaka_link
            link_rows = conn.execute(
                """SELECT sinhala_filename, sinhala_page_num,
                          sinhala_entry_order, sinhala_palitext,
                          nissaya_title, match_confidence
                   FROM epitaka_link
                   WHERE nissaya_book_id = ? AND nissaya_para_id = ?
                   ORDER BY match_confidence DESC""",
                (book_id, para_id)
            ).fetchall()
            if not link_rows:
                log_warn(
                    f"[DB] No epitaka_link for {book_id}/para_id={para_id}."
                )
                return {"title": f"{book_id}:{para_id}",
                        "pali_block": "", "sinhala_block": ""}

            title = link_rows[0]["nissaya_title"] or f"{book_id}:{para_id}"

            pali_parts    = []
            sinhala_parts = []
            seen_pages: set = set()

            for link in link_rows:
                fn  = link["sinhala_filename"]
                pn  = link["sinhala_page_num"]
                eo  = link["sinhala_entry_order"]
                key = (fn, pn)
                if key in seen_pages:
                    continue
                seen_pages.add(key)

                # 1. Find the heading level of the anchor entry (fn, pn, eo)
                anchor = conn.execute(
                    """SELECT page_order, entry_order, level
                       FROM entries
                       WHERE filename = ? AND page_num = ? AND entry_order = ?""",
                    (fn, pn, eo)
                ).fetchone()

                if anchor is None:
                    # Fallback: just grab the whole page as before
                    entries = conn.execute(
                        """SELECT palitext, sinhalatext, type
                           FROM entries
                           WHERE filename = ? AND page_num = ?
                           ORDER BY page_order, entry_order""",
                        (fn, pn)
                    ).fetchall()
                else:
                    anchor_page_order  = anchor["page_order"]
                    anchor_entry_order = anchor["entry_order"]
                    anchor_level       = anchor["level"]  # may be None if not a heading

                    # 2. Find the next heading at the same or higher level
                    #    that comes strictly after the anchor position.
                    #    "Higher level" means a smaller or equal level number
                    #    (e.g. level 1 is a chapter, level 2 a section, etc.).
                    #    We want rows where type='heading' AND level <= anchor_level
                    #    AND (page_order, entry_order) > anchor position.
                    if anchor_level is not None:
                        stop_row = conn.execute(
                            """SELECT page_order, entry_order
                               FROM entries
                               WHERE filename = ?
                                 AND type = 'heading'
                                 AND level >= ?
                                 AND (
                                     page_order > ?
                                     OR (page_order = ? AND entry_order > ?)
                                 )
                               ORDER BY page_order, entry_order
                               LIMIT 1""",
                            (fn, anchor_level,
                             anchor_page_order,
                             anchor_page_order, anchor_entry_order)
                        ).fetchone()
                    else:
                        stop_row = None  # anchor has no level; read to end of file

                    # 3. Fetch everything from the anchor up to (not including)
                    #    the stop heading, across as many pages as needed.
                    if stop_row is None:
                        # No closing heading found — read to end of the file
                        entries = conn.execute(
                            """SELECT palitext, sinhalatext, type
                               FROM entries
                               WHERE filename = ?
                                 AND (
                                     page_order > ?
                                     OR (page_order = ? AND entry_order >= ?)
                                 )
                               ORDER BY page_order, entry_order""",
                            (fn,
                             anchor_page_order,
                             anchor_page_order, anchor_entry_order)
                        ).fetchall()
                    else:
                        stop_po = stop_row["page_order"]
                        stop_eo = stop_row["entry_order"]
                        entries = conn.execute(
                            """SELECT palitext, sinhalatext, type
                               FROM entries
                               WHERE filename = ?
                                 AND (
                                     page_order > ?
                                     OR (page_order = ? AND entry_order >= ?)
                                 )
                                 AND NOT (
                                     page_order > ?
                                     OR (page_order = ? AND entry_order >= ?)
                                 )
                               ORDER BY page_order, entry_order""",
                            (fn,
                             anchor_page_order,
                             anchor_page_order, anchor_entry_order,
                             stop_po,
                             stop_po, stop_eo)
                        ).fetchall()

                for e in entries:
                    if e["palitext"] and e["palitext"].strip():
                        pali_parts.append(e["palitext"].strip())
                    if e["sinhalatext"] and e["sinhalatext"].strip():
                        sinhala_parts.append(e["sinhalatext"].strip())

            pali_block    = "\n".join(pali_parts)
            sinhala_block = "\n".join(sinhala_parts)
            log_info(
                f"[DB] Sinhala reference for {book_id}/{para_id}: "
                f"title={title!r}, "
                f"pali={len(pali_block)} chars, "
                f"sinhala={len(sinhala_block)} chars."
            )
            return {
                "title":         title,
                "pali_block":    pali_block,
                "sinhala_block": sinhala_block,
            }
    except Exception as exc:
        log_warn(f"[DB] fetch_sinhala_reference failed: {exc}")
        return {"title": f"{book_id}:{para_id}",
                "pali_block": "", "sinhala_block": ""}

# ══════════════════════════════════════════════════════════════════════════════
# Nissaya
# ══════════════════════════════════════════════════════════════════════════════

def fetch_nissaya_map(
    params:   dict,
    book_id:  str,
    para_id:  int,
    log_info: _Log,
) -> dict[int, str]:
    """Return {line_id: nissaya_content} for the given paragraph."""
    with _connect(nissaya_path(params)) as conn:
        rows = conn.execute(
            """SELECT line_id, content
               FROM nissaya
               WHERE book_id = ? AND para_id = ?
               ORDER BY line_id""",
            (book_id, para_id)
        ).fetchall()

    result = {r["line_id"]: (r["content"] or "") for r in rows}
    # log_info(f"[DB]   nissaya rows for para_id={para_id}: {len(result)}")
    return result


def build_nissaya_block(sentences: list[dict], nissaya_map: dict[int, str]) -> str:
    """
    Build a human-readable nissaya block aligned by line_id.

      [line_id=1] <pali_sentence>
        Nissaya: <gloss>
    """
    if not nissaya_map:
        return "(no nissaya available for this paragraph)"

    lines = []
    for s in sentences:
        lid       = s["line_id"]
        niss_text = (nissaya_map.get(lid) or "").strip() or "(none)"
        lines.append(
            f"[line_id={lid}] {s.get('pali_sentence', '')}\n"
            f"  Nissaya: {niss_text}"
        )
    return "\n\n".join(lines)


# ══════════════════════════════════════════════════════════════════════════════
# Glossary — read (sinhala_glossary table)
# ══════════════════════════════════════════════════════════════════════════════

def extract_pali_tokens(text: str) -> list[str]:
    tokens = re.split(r"[\s,;.\u2018\u2019\"'()\[\]]+", text)
    return list({t.lower() for t in tokens if 2 < len(t) < 40})

def fetch_glossary_block(
    pali_tokens: list[str],
    log_info:    _Log,
    log_warn:    _Log,
) -> str:
    """
    Look up matching entries in sinhala_glossary and return a formatted
    string for inclusion in the prompt.
    """
    if not pali_tokens:
        return "(no existing glossary entries)"

    CHUNK = 999  # SQLite SQLITE_MAX_VARIABLE_NUMBER default
    try:
        conn = get_glossary_conn()
        try:
            rows = []
            for i in range(0, len(pali_tokens), CHUNK):
                chunk = pali_tokens[i : i + CHUNK]
                placeholders = ",".join("?" * len(chunk))
                rows += conn.execute(
                    f"SELECT pali, sinhala, context FROM sinhala_glossary "
                    f"WHERE pali IN ({placeholders})",
                    chunk,
                ).fetchall()
        finally:
            conn.close()
    except Exception as exc:
        log_warn(f"[DB] sinhala_glossary lookup failed (non-fatal): {exc}")
        return "(glossary unavailable)"

    if not rows:
        return "(no matching glossary entries yet)"

    log_info(f"[DB] Glossary: {len(rows)} matching term(s) loaded.")
    lines = []
    for r in rows:
        context = r["context"] or ""
        line = f"  {r['pali']} → {r['sinhala']}"
        if context:
            line += f"  [{context}]"
        lines.append(line)
    return "\n".join(lines)

# ══════════════════════════════════════════════════════════════════
# Commentary & Sub-commentary (via book_links)
# ══════════════════════════════════════════════════════════════════

def fetch_commentary_block(
    params:    dict,
    src_lines: list[tuple],   # (book_id, para_id, line_id) — full composite key
    max_chars: int = 100000,
    log_info:  _Log = None,
    log_warn:  _Log = None,
) -> str:
    """
    Fetch commentary (atthakatha) and sub-commentary (tika) text linked to
    the given source sentences via the book_links table.

    src_lines : list of (src_book, src_para, src_line) tuples.
                line_id alone is NOT globally unique — it is only the row
                order within one paragraph, so we must match all three columns.

    Three-tier fallback when result exceeds max_chars:

      Tier 1 — Full paragraphs
        Fetch every sentence in every dst paragraph that has at least one
        link. Duplicate paragraphs are collapsed.

      Tier 2 — 3-line window (i-1, i, i+1)
        For every directly-linked dst_line, fetch that line plus its
        immediate neighbours. Overlapping windows are deduplicated.

      Tier 3 — Exact linked lines only (i)
        Keep only the single dst_line that is directly referenced.

    All bulk DB lookups use TEMP TABLE JOINs to avoid SQLite's
    bound-variable limit.
    """
    if log_info is None:
        log_info = lambda x: None
    if log_warn is None:
        log_warn = lambda x: None

    if not src_lines:
        return "(no commentary available)"

    path = nissaya_path(params)

    try:
        with _connect(path) as conn:

            # 0. Guard: check book_links exists
            has_table = conn.execute(
                "SELECT 1 FROM sqlite_master WHERE type='table' AND name='book_links'"
            ).fetchone()
            if not has_table:
                log_warn("[DB] book_links table not found. Skipping commentary fetch.")
                return "(no commentary available)"

            # 1. Load source keys into a temp table, then JOIN book_links.
            #    Matching on all three columns (src_book, src_para, src_line)
            #    because line_id is only unique within its own paragraph.
            conn.execute("DROP TABLE IF EXISTS _tmp_src")
            conn.execute(
                "CREATE TEMP TABLE _tmp_src"
                " (book_id TEXT, para_id INTEGER, line_id INTEGER)"
            )
            conn.executemany("INSERT INTO _tmp_src VALUES (?,?,?)", src_lines)

            link_rows = conn.execute(
                """SELECT bl.src_book, bl.src_para, bl.src_line,
                          bl.dst_book, bl.dst_para, bl.dst_line
                   FROM   book_links bl
                   JOIN   _tmp_src s
                          ON  bl.src_book = s.book_id
                          AND bl.src_para = s.para_id
                          AND bl.src_line = s.line_id
                   ORDER  BY bl.dst_book, bl.dst_para, bl.dst_line"""
            ).fetchall()
            conn.execute("DROP TABLE IF EXISTS _tmp_src")

            if not link_rows:
                log_info(
                    f"[DB] Commentary: no book_links for {len(src_lines)} source line(s)."
                )
                return "(no commentary available)"

            log_info(
                f"[DB] Commentary: {len(link_rows)} link row(s) for "
                f"{len(src_lines)} source line(s)."
            )

            # Unique (dst_book, dst_para, dst_line) targets, order preserved.
            seen_targets: dict[tuple, None] = {}
            for r in link_rows:
                seen_targets[(r["dst_book"], r["dst_para"], r["dst_line"])] = None
            targets = list(seen_targets.keys())
            log_info(f"[DB] Commentary: {len(targets)} unique target(s) after dedup.")

            # Helpers: bulk fetches via TEMP TABLE

            def _fetch_exact(triples: list[tuple]) -> list[sqlite3.Row]:
                """Fetch sentences for explicit (book_id, para_id, line_id) triples."""
                if not triples:
                    return []
                conn.execute("DROP TABLE IF EXISTS _tmp_targets")
                conn.execute(
                    "CREATE TEMP TABLE _tmp_targets"
                    " (book_id TEXT, para_id INTEGER, line_id INTEGER)"
                )
                conn.executemany("INSERT INTO _tmp_targets VALUES (?,?,?)", triples)
                rows = conn.execute(
                    """SELECT s.book_id, s.para_id, s.line_id, s.pali_sentence
                       FROM   sentences s
                       JOIN   _tmp_targets t
                              ON  s.book_id = t.book_id
                              AND s.para_id = t.para_id
                              AND s.line_id = t.line_id
                       ORDER  BY s.book_id, s.para_id, s.line_id"""
                ).fetchall()
                conn.execute("DROP TABLE IF EXISTS _tmp_targets")
                return rows

            def _fetch_paragraphs(pairs: list[tuple]) -> list[sqlite3.Row]:
                """Fetch all sentences for (book_id, para_id) pairs."""
                if not pairs:
                    return []
                conn.execute("DROP TABLE IF EXISTS _tmp_paras")
                conn.execute(
                    "CREATE TEMP TABLE _tmp_paras (book_id TEXT, para_id INTEGER)"
                )
                conn.executemany("INSERT INTO _tmp_paras VALUES (?,?)", pairs)
                rows = conn.execute(
                    """SELECT s.book_id, s.para_id, s.line_id, s.pali_sentence
                       FROM   sentences s
                       JOIN   _tmp_paras t
                              ON  s.book_id = t.book_id
                              AND s.para_id = t.para_id
                       ORDER  BY s.book_id, s.para_id, s.line_id"""
                ).fetchall()
                conn.execute("DROP TABLE IF EXISTS _tmp_paras")
                return rows

            def _render(rows: list[sqlite3.Row], label: str) -> str:
                if not rows:
                    return ""
                sections: dict[tuple, list[str]] = {}
                for r in rows:
                    key = (r["book_id"], r["para_id"])
                    sections.setdefault(key, []).append(
                        f"  [{r['line_id']}] {r['pali_sentence'] or ''}"
                    )
                parts = [label]
                for (book, para), sent_lines in sections.items():
                    parts.append(f"\n[{book} §{para}]")
                    parts.extend(sent_lines)
                return "\n".join(parts)

            # Tier 1: full paragraphs
            unique_paras: dict[tuple, None] = {}
            for dst_book, dst_para, _ in targets:
                unique_paras[(dst_book, dst_para)] = None
            log_info(
                f"[DB] Commentary tier-1: fetching {len(unique_paras)} unique paragraph(s)."
            )
            tier1_rows = _fetch_paragraphs(list(unique_paras.keys()))
            tier1_text = _render(tier1_rows, "[Commentary & Sub-commentary - full paragraphs]")

            if max_chars <= 0 or len(tier1_text) <= max_chars:
                log_info(
                    f"[DB] Commentary tier-1 OK: "
                    f"{len(tier1_text)} chars, {len(tier1_rows)} sentence(s)."
                )
                return tier1_text or "(no commentary available)"

            # Tier 2: 3-line window (i-1, i, i+1)
            log_info(
                f"[DB] Commentary tier-1 too long ({len(tier1_text)} > {max_chars}); "
                f"falling back to 3-line window."
            )
            window_set: dict[tuple, None] = {}
            for dst_book, dst_para, dst_line in targets:
                for offset in (-1, 0, 1):
                    window_set[(dst_book, dst_para, dst_line + offset)] = None
            log_info(f"[DB] Commentary tier-2: {len(window_set)} window line(s) to fetch.")
            tier2_rows = _fetch_exact(list(window_set.keys()))
            tier2_text = _render(tier2_rows, "[Commentary & Sub-commentary - 3-line window]")

            if len(tier2_text) <= max_chars:
                log_info(
                    f"[DB] Commentary tier-2 OK: "
                    f"{len(tier2_text)} chars, {len(tier2_rows)} sentence(s)."
                )
                return tier2_text or "(no commentary available)"

            # Tier 3: exact linked lines only
            log_info(
                f"[DB] Commentary tier-2 too long ({len(tier2_text)} > {max_chars}); "
                f"falling back to exact linked lines only."
            )
            log_info(f"[DB] Commentary tier-3: {len(targets)} exact target(s).")
            tier3_rows = _fetch_exact(targets)
            tier3_text = _render(tier3_rows, "[Commentary & Sub-commentary - linked lines only]")

            # ADD THIS:
            if max_chars > 0 and len(tier3_text) > max_chars:
                tier3_text = tier3_text[:max_chars]+'...'
                log_warn(f"[DB] Commentary tier-3 truncated to {max_chars} chars.")

            log_info(
                f"[DB] Commentary tier-3 final: "
                f"{len(tier3_text)} chars, {len(tier3_rows)} sentence(s)."
            )

            return tier3_text or "(no commentary available)"

    except Exception as exc:
        log_warn(f"[DB] Commentary fetch failed: {exc}")
        return "(no commentary available)"


# ══════════════════════════════════════════════════════════════════════════════
# Pāli word definitions
# ══════════════════════════════════════════════════════════════════════════════

def fetch_pali_definitions_block(
    pali_text: str,
    params:    dict,
    log_info:  _Log,
    log_warn:  _Log,
) -> str:
    """
    Look up words from pali_text in the nissaya pali_definition table and
    return example sentences for difficult/rare words.
    """
    words = re.findall(r"\b[\w\u0900-\u097F]{5,}\b", pali_text)
    words = list(set(w.lower() for w in words))

    if not words:
        return "(no word definitions available)"

    try:
        path = nissaya_path(params)
        with _connect(path) as conn:
            tables = conn.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='pali_definition'"
            ).fetchone()
            if not tables:
                log_warn("[DB] pali_definition table not found.")
                return "(no word definitions available)"

            defs_block = []
            for word in words[:30]:
                stem_rows = conn.execute(
                    "SELECT DISTINCT stem FROM pali_definition "
                    "WHERE plain = ? OR word = ? LIMIT 1",
                    (word, word)
                ).fetchall()

                for (stem,) in (stem_rows or []):
                    usage_rows = conn.execute(
                        """SELECT s.pali_sentence
                           FROM pali_definition pd
                           JOIN sentences s ON s.book_id = pd.book_id
                                           AND s.para_id = pd.para_id
                                           AND s.line_id = pd.line_id
                           WHERE pd.stem = ?
                           LIMIT 1""",
                        (stem,)
                    ).fetchall()
                    if usage_rows:
                        example = " … ".join(r["pali_sentence"] for r in usage_rows)
                        defs_block.append(f"  {word} (stem: {stem}): {example}")

            if defs_block:
                result = "Pāli Word Definitions:\n" + "\n".join(defs_block)
                log_info(f"[DB] Pali definitions: {len(defs_block)} word(s) defined.")
                return result
            return "(no word definitions found)"

    except Exception as exc:
        log_warn(f"[DB] Pali definitions lookup failed: {exc}")
        return "(word definitions unavailable)"


# ══════════════════════════════════════════════════════════════════════════════
# Previous paragraph context
# ══════════════════════════════════════════════════════════════════════════════

def fetch_previous_paragraph_translation(
    params:   dict,
    book_id:  str,
    para_id:  int,
    log_info: _Log,
) -> str:
    """
    Fetch the previous paragraph (para_id - 1) from epitaka_si.db if it is
    already translated.  Used so the AI maintains consistent style.
    """
    if para_id <= 0:
        return ""

    path = output_path(params)
    try:
        with _connect(path) as conn:
            rows = conn.execute(
                """SELECT line_id, sinhala_translation
                   FROM sentences
                   WHERE book_id = ? AND para_id = ?
                     AND sinhala_translation IS NOT NULL
                     AND sinhala_translation != ''
                   ORDER BY line_id""",
                (book_id, para_id - 1)
            ).fetchall()

        if rows:
            prev_text = " ".join(r["sinhala_translation"].strip() for r in rows)
            log_info(
                f"[DB] Previous paragraph (para_id={para_id - 1}): "
                f"{len(prev_text)} chars."
            )
            return f"[Previous Paragraph {para_id - 1}]\n{prev_text}"
        return ""

    except Exception as exc:
        log_info(f"[DB] Could not fetch previous paragraph: {exc}")
        return ""


# ══════════════════════════════════════════════════════════════════════════════
# Translations — write
# ══════════════════════════════════════════════════════════════════════════════

def save_translations(
    params:       dict,
    book_id:      str,
    para_id:      int,
    translations: list[dict],
    log_info:     _Log,
    log_warn:     _Log,
    log_error:    _Log,
) -> int:
    """
    Upsert rows into epitaka_si.db sentences table.
    Each translation entry must have: line_id, pali_sentence (optional),
    sinhala_translation.
    Returns the number of rows inserted or updated.
    """
    updated = 0
    path    = output_path(params)

    with _connect(path) as conn:
        for entry in translations:
            line_id = entry.get("line_id")
            text    = str(entry.get("sinhala_translation") or "").strip()
            pali    = str(entry.get("pali_sentence") or "").strip()

            if line_id is None or not text:
                log_warn(f"[DB] Skipping empty/malformed entry: {entry}")
                continue

            try:
                conn.execute(
                    """INSERT INTO sentences
                         (book_id, para_id, line_id, pali_sentence,
                          sinhala_translation, updated_at)
                       VALUES (?, ?, ?, ?, ?, datetime('now'))
                       ON CONFLICT(book_id, para_id, line_id)
                       DO UPDATE SET
                         sinhala_translation = excluded.sinhala_translation,
                         updated_at          = excluded.updated_at""",
                    (book_id, para_id, line_id, pali, text)
                )
                updated += conn.execute("SELECT changes()").fetchone()[0]
            except Exception as exc:
                log_error(f"[DB] Error saving line_id={line_id}: {exc}")

        conn.commit()

    log_info(
        f"[DB] save_translations: {updated} row(s) saved for "
        f"book_id={book_id!r}, para_id={para_id}."
    )
    return updated


# ══════════════════════════════════════════════════════════════════════════════
# Glossary — write (sinhala_glossary table)
# ══════════════════════════════════════════════════════════════════════════════

def upsert_glossary(
    terms:     list[dict],
    source_id: str,
    log_info:  _Log,
    log_warn:  _Log,
    log_error: _Log,
) -> int:
    """
    Insert new Pāli→Sinhala glossary terms into sinhala_glossary table.
    Required fields per term: pali, sinhala.
    Returns the count of rows actually inserted.
    """
    required = {"pali", "sinhala"}
    inserted = 0

    try:
        conn = get_glossary_conn()
    except Exception as exc:
        log_warn(f"[DB] Cannot open glossary DB: {exc}")
        return 0

    try:
        for term in terms:
            if not required.issubset(term):
                log_warn(f"[DB] Skipping malformed glossary entry: {term}")
                continue
            pali    = str(term.get("pali",    "")).strip()
            sinhala = str(term.get("sinhala", "")).strip()
            if not pali or not sinhala:
                continue
            try:
                conn.execute(
                    """INSERT INTO sinhala_glossary
                         (pali, sinhala, domain, sub_domain, context, note, source_id)
                       VALUES (?, ?, ?, ?, ?, ?, ?)
                       ON CONFLICT(pali, sinhala) DO NOTHING""",
                    (
                        pali, sinhala,
                        str(term.get("domain",     "") or ""),
                        str(term.get("sub_domain", "") or ""),
                        str(term.get("context",    "") or ""),
                        str(term.get("note",       "") or ""),
                        source_id,
                    )
                )
                inserted += conn.execute("SELECT changes()").fetchone()[0]
            except Exception as exc:
                log_error(f"[DB] Glossary insert error for '{pali}': {exc}")

        conn.commit()
    finally:
        conn.close()

    log_info(
        f"[DB] upsert_glossary: {inserted} new term(s) (source_id={source_id!r})."
    )
    return inserted
