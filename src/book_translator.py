...
PRESET_BOOKS = (
    ...
    "Cp-a,Ja-a-i,Ja-a-ii,Ja-a-iii,Ja-a-iv,Ja-a-v,Ja-a-vi-b,Ja-a-vi,Ja-a-vii,Ja-ṭ-iv,Ja-ṭ-iii,Ja-ṭ-v,"
    "Ja-ṭ-i,Ja-ṭ-ii,""Nidd-a-i,Nidd-a-ii,"
...
# Legacy explicit override only (cu.GLOSSARY_DB defaults to "" — there is no
# bare glossary.db anymore; every real path uses glossary_<lang>.db).
...
# model's quota is exhausted for every key (3x HTTP 429 per key) it is
...
# this size nothing is touched — full commentary, full parallel-translation
# set, full chunk — so normal-sized requests never lose any quality.
#
# Measured in ESTIMATED TOKENS (ai.estimate_tokens), not raw UTF-8 bytes.
# Bytes are a poor proxy for token count once Thai/Sinhala/Myanmar reference
# text is involved: those scripts are multi-byte per character but often
# tokenize close to 1 token/char, so a byte-based cap can let a prompt through
# that is actually far bigger in real tokens than intended. This is meant to
# line up with the actual per-request token limit of the API/account in use
# (default here: 200k).
PROMPT_TOKEN_LIMIT = int(os.environ.get("PROMPT_TOKEN_LIMIT", 200_000))

# Chunks whose PENDING sentence count is below this are merged into the next
# chunk (or, if last, the previous one) rather than sent as their own tiny
# API call. This matters most on partial re-runs, where most of a book is
# already translated and only a handful of lines are still pending per
# section — without merging, each such leftover fires its own call with the
# full fixed prompt overhead (glossary/commentary/instructions) for as little
# as one sentence. Merged chunks are still subject to PROMPT_TOKEN_LIMIT
# above and get split back apart if the merge pushes them over budget.
MIN_CHUNK_PENDING_LINES = int(os.environ.get("MIN_CHUNK_PENDING_LINES", 5))
...
# reading from the fallback path no matter what --lang is. So we must
...
if _early_glossary_db:
    ...
    # No --lang available yet (e.g. introspection/help); leave unconfigured.
    # main() always re-resolves this to glossary_<lang>.db before anything
    # reads it, so this empty value is never used for real work.
...
# Script-bleed guard
#
# NOTE: the actual detection logic lives in common_utils
# (find_context_script_bleed / describe_context_script_bleed) so that both
# the live translation check here AND cu.save_glossary_terms() share one
# definition (previously the glossary side had no check at all, which is
# how a contaminated glossary entry could survive and keep re-poisoning
# every later chunk via "apply exactly"). The prompt's reference context
# includes Thai, Sinhala, Myanmar, and English text, so all three non-Latin
# reference scripts are checked against every target language that isn't
# that script itself — not just Thai, and not just a hand-picked
# "confusable" list.
# ══════════════════════════════════════════════════════════════════

def check_translations_for_script_bleed(lang: str, translations: list[dict]) -> list[dict]:
    """
    Scan a parsed 'translations' list for entries that look like they bled
    into Thai/Sinhala/Myanmar/Devanagari script instead of the target language.
    Flagged entries have their "translation" text BLANKED OUT (not merely
    downgraded) so that save_translations_to_lang_db() will skip persisting
    them — the line stays "pending" and will be retried on a future run,
    instead of a wrong-script string being written permanently into the DB.
    The original bad text is preserved in confidence_note for logging/
    debugging. Returns the list of flagged entries so the caller can
    log/report them.
    """
    flagged = []
    for t in translations:
        ...
        if reason:
            ...
            t["confidence"] = "low"
            t["translation"] = ""   # drop the bad text — leave the line pending for retry
            flagged.append(t)
    return flagged


def check_glossary_terms_for_script_bleed(lang: str, new_terms: list[dict]) -> tuple[list[dict], list[dict]]:
    """
    Split a parsed 'glossary' list into (clean, flagged) based on whether
    the "translation" field looks like Thai/Sinhala/Myanmar/Devanagari script bleed.
    This is a first-line filter run in book_translator.py itself;
    save_glossary_terms() in common_utils.py additionally re-checks
    (defense in depth) since it's also called from glossary_builder.py's
    separate code path.
    """
    clean, flagged = [], []
...
def merge_small_chunks(
    chunks: list[list[dict]],
    min_lines: int = MIN_CHUNK_PENDING_LINES,
) -> list[list[dict]]:
    """
    Merge any chunk whose total PENDING sentence count is below `min_lines`
    into the following chunk, so a leftover of a few scattered lines doesn't
    fire its own API call with the full fixed prompt overhead. If the very
    last chunk ends up too small (nothing after it to merge into), it's
    folded into the previous chunk instead.

    This runs AFTER chunk_paragraphs(), so token-budget grouping has already
    happened; merged results can still exceed PROMPT_TOKEN_LIMIT once real
    context blocks are built, in which case _handle_chunk's existing
    size-reduction cascade splits them back apart as needed.
    """
    if not chunks:
        return chunks
    ...
    for chunk in chunks:
        combined = carry + chunk if carry else chunk
        n_lines = sum(len(p["pending"]) for p in combined)
...
ACCURATE and READABLE for a GENERAL but serious audience. When selecting terminology,
...
def process_book(
    ...
    # NOTE: "glossary_db" must be in here — _handle_chunk builds
    # GlossaryContextStemmed(params, ...) from this dict, and without the key
    # every prompt silently gets "(no glossary DB configured)" instead of the
    # established-glossary block (this exact omission happened before).
    ...
    for part_idx, part in enumerate(sections, 1):
        ...
        n_before_merge = len(chunks)
        chunks = merge_small_chunks(chunks)
        if len(chunks) != n_before_merge:
            print(f"  -> {n_before_merge} chunk(s), merged down to {len(chunks)} "
                  f"(chunks under {MIN_CHUNK_PENDING_LINES} pending line(s) folded into a neighbor)")
        else:
        ...
        def _handle_chunk(chunk: list[dict], depth: int = 0) -> tuple[int, int, int]:
            ...
            # being sent to translate in this chunk — NOT the chunk_start..
            # chunk_end para range. When most of a book is already translated,
            # the handful of remaining pending lines can be scattered far
            # apart, making chunk_start..chunk_end span huge stretches (even
            # the whole book). Fetching by range there used to pull every
            # already-translated line in en/th/si across that whole span
            # into the prompt, which is how partial re-runs blew past the
            # token budget.
            chunk_line_ids = [
                (para["para_id"], s["line_id"])
                for para in chunk
                for s in para["pending"]
            ]
            ...
            local_pali_defs_block = PaliDefsContext(
                params, pali_text=pali_text_for_chunk, max_words=50, log_info=print, log_warn=print,
            ).build()
            ...
            def _build():
                return build_prompt(
                    ...
                    nissaya_block=nissaya_block, parallel_block=parallel_block,
                )

            prompt, flat_sentences = _build()
            size_bytes = _prompt_bytes(system_prompt, prompt)
            n_tokens = ai.estimate_tokens(system_prompt + prompt)

            if n_tokens > PROMPT_TOKEN_LIMIT:
                halves = _split_chunk_in_half(chunk)
                if halves is not None and depth < 8:
                    print(f"  [size] p{chunk_start}-{chunk_end}: still ~{n_tokens:,} tokens "
                ...
                print(f"  [size] p{chunk_start}-{chunk_end}: ~{n_tokens:,} tokens with a single "
                      f"pending sentence — truncating commentary/word-def context "
                      f"as a last resort (translation quality may be affected here)")
                cap_chars = (PROMPT_TOKEN_LIMIT // 3) * 4  # rough tokens->chars back-conversion
                local_commentary_block = local_commentary_block[:cap_chars]
                local_pali_defs_block  = local_pali_defs_block[:cap_chars]
                prompt, flat_sentences = _build()
                size_bytes = _prompt_bytes(system_prompt, prompt)
                n_tokens = ai.estimate_tokens(system_prompt + prompt)
            ...
            print(f"  Chunk {chunk_label}: {n_sentences} sentence(s) "
                  f"across {len(chunk)} para(s), {size_bytes:,} bytes (~{n_tokens:,} tokens)")
            ...
            new_terms, gloss_bleed_flagged = check_glossary_terms_for_script_bleed(args.lang, new_terms)
            ...
            print(f"  Parsed: {len(translations)} translation(s) "
                  f"({low_conf} low-confidence, {len(bleed_flagged)} script-bleed dropped), "
                  f"{len(new_terms)} glossary term(s) ({len(gloss_bleed_flagged)} script-bleed dropped), "
...
def save_translations_to_lang_db(lang_db: str, book_id: str, translations: list[dict]) -> int:
    """
    Upsert translated sentences (+ confidence/confidence_note) into the
    lang-specific DB. Entries with an empty/blank "translation" (e.g. ones
    just dropped by check_translations_for_script_bleed) are SKIPPED
    entirely rather than written as an empty string — writing an empty
    string would both (a) permanently mark the sentence as "already
    translated" so it's never retried, since pending-detection checks for
    translation IS NOT NULL AND != '', and (b) risk clobbering a
    previously-good translation already sitting in the DB from an earlier
    run.
    """
    if not translations:
        return 0
    rows = [
        ...
        for t in translations
        if "para_id" in t and "line_id" in t and (t.get("translation") or "").strip()
    ]
    if not rows:
        return 0
...
def main() -> int:
    ...
    # chain (first = preferred). ai_client.ModelPool drops a model once every
    # key is 429-parked on it and moves to the next one automatically.
    models = [args.model] if args.model else list(FALLBACK_MODEL_CHAIN)
    print(f"[model] Using model pool: {', '.join(models)}")
    ...
    glossary_writer    = GlossaryWriter(params, log_info=print, log_warn=print, log_error=print)
